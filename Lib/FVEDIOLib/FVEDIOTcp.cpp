...
namespace Utilities
{
	namespace FVEDIO
	{
...
#endif
		void FVEDIOTcp::Query(UINT8 WriteData[], UINT16 WriteCount, UINT8 ReadData[], PUINT32 ReadCount)
		{
			CAutoLock al(m_FVEDIOTcpLock);
			...
			*ReadCount = 0;
			...
			if (wdataSize>2048)
			{
				...
				throw FVEDIOExc(GetIPaddress(), msg);
			}
			...
			{
				throw FVEDIOExc(GetIPaddress(), (const char*)e);
			}
			...
			{
				char msg[256];
				sprintf_s(msg, sizeof(msg), "FVEDIO no response");
				throw FVEDIOExc(GetIPaddress(), msg);
			}
		...
		void FVEDIOTcp::Write(UINT8 WriteData[], UINT16 WriteCount)
		{
