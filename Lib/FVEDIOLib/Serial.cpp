...
#define COMM_EVTs	ARRAY_CNT(gs_comm_evt)
		...
		void Serial::Connect(const char* COMPort, BaudRate Baudrate, DataBits Databits, Parity parity, StopBits Stopbits)
		{
			...
			if (fh == INVALID_HANDLE_VALUE)
			{
				...
				sprintf_s(err_msg, sizeof(err_msg), "CreateFile() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			m_Fh = fh;
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "GetCommState() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "SetCommState() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			m_Dcb = dcb;
			...
			if (!m_Overlapped.ReadWriteFile.hEvent)
			{
				...
				char	err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "CreateEvent() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			...
			do
			{
				...
				if (!succeed)
				{
					...
					default:
					{
						...
						sprintf_s(err_msg, sizeof(err_msg), "ReadFile() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
						throw err_msg;
					}
					}
				}
		...
		void Serial::serial_write(UINT8 WriteData[], UINT32 WriteCount)
		{
			...
			if (!succeed)
			{
				...
				default:
				{
					char err_msg[256];
					DWORD dwLastError = le;
					sprintf_s(err_msg, sizeof(err_msg), "WriteFile() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
					throw err_msg;
				}
				}
			}
		...
		void Serial::ols_wait_for_overlapped(LPOVERLAPPED p_overlapped, DWORD *p_len, DWORD timeout)
		{
			...
			case WAIT_OBJECT_0:
				...
				if (!succeed)
				{
					char err_msg[256];
					DWORD dwLastError = GetLastError();
					sprintf_s(err_msg, sizeof(err_msg), "GetOverlappedResult() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
					throw err_msg;
				}
				break;
			case WAIT_TIMEOUT:
			...
			case WAIT_FAILED:
			{
				char err_msg[256];
				DWORD dwLastError = GetLastError();
				sprintf_s(err_msg, sizeof(err_msg), "WaitForSingleObject() failed: %d; %s", dwLastError, GetLastErrorString(dwLastError));
				throw err_msg;
			}
			break;
			default:
		...
		void Serial::ols_wait_comm_event(DWORD *pmask, UINT32 timeout)
		{
			...
			if (!succeed)
			{
				...
				default:
				{
					char err_msg[256];
					DWORD dwLastError = le;
					sprintf_s(err_msg, sizeof(err_msg), "WaitCommEvent() error: %d; %s", dwLastError, GetLastErrorString(dwLastError));
					throw err_msg;
				}
				}
			}
